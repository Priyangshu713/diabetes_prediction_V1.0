...
export default function RiskResult() {
  const { risk, resetAssessment } = useRiskStore();

  const getRiskLevel = () => {
    if (risk >= 0.7) return { level: 'High', color: 'text-red-400', bg: 'bg-red-400/10' };
    if (risk >= 0.3) return { level: 'Moderate', color: 'text-yellow-400', bg: 'bg-yellow-400/10' };
