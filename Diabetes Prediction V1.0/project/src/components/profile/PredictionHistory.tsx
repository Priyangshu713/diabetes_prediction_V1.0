...
import { X, TrendingDown, TrendingUp, ChevronRight, ArrowLeft } from 'lucide-react';
          ...
          <div className="p-4 space-y-4">
            {selectedRecord ? (
              ...
              mockHistory.map((record) => (
                ...
                >
                  <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-b ${getRiskColor(record.risk)} p-4 border border-white/5`}>
                    {/* Top Section */}
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <div className="text-sm text-gray-400">{formatTimeAgo(record.date)}</div>
                      ...
                      <div className="flex items-center gap-2 bg-gray-900/30 rounded-full px-3 py-1">
                        <span className="text-lg font-semibold text-white">
                          {(record.risk * 100).toFixed(0)}%
                        </span>
                        {record.trend === 'up' && <TrendingUp className="w-4 h-4 text-red-400" />}
                        {record.trend === 'down' && <TrendingDown className="w-4 h-4 text-green-400" />}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div className="bg-gray-900/30 rounded-xl p-3">
                        ...
                        <div className="text-lg font-medium text-white">{record.glucose} <span className="text-sm">mg/dL</span></div>
                      </div>
                    </div>
